...
GitHub `chu4urdolen/noobia` is the canonical source. This project now lives in
the repository `council/` directory; run its build and install commands from
that directory. Nexus, Argus, and Rose update it with `git pull --ff-only`.
Do not copy source trees directly between machines.
