...
void CommandDispatcher::setVmLifecycle(NoobVmLifecycle *lifecycle) {
...
String CommandDispatcher::dispatch(const NoobRequest &request) {
  ...
  if (request.command == "RUN" || request.command == "START_THREAD") {
    ...
    if (lifecycle_) {
      const String detail = lifecycle_->onStarted(vm_);
      if (!detail.isEmpty()) payload += " " + detail;
    }
    return NoobProtocol::ok(request.requestId, payload);
  }
  if (request.command == "STOP" || request.command == "STOP_THREAD") {
    vm_.stop();
    String payload = vm_.status();
    if (lifecycle_) {
      const String detail = lifecycle_->onStopped(vm_);
      if (!detail.isEmpty()) payload += " " + detail;
    }
    return NoobProtocol::ok(request.requestId, payload);
  }
  if (request.command == "RESET_VM") {
    ...
    String payload = vm_.status();
    if (lifecycle_) {
      const String detail = lifecycle_->onReset(vm_);
      if (!detail.isEmpty()) payload += " " + detail;
    }
    return NoobProtocol::ok(request.requestId, payload);
  }
